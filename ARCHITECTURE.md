...
5. **Material-or-axes routing.** When `AnalysisResult.material` is one of the five canonical values (WOOD / PAPER / ROCKS / SAND / FABRIC), the polished hand-tuned path runs. When it is null, the axis-driven procedural path runs. This single null-check is the entire branching logic for the dynamic-material story.
...
│   └── model/
│       ├── Material.kt                    enum WOOD/PAPER/ROCKS/SAND/FABRIC. Null = open vocabulary.
...
enum class Material { WOOD, PAPER, ROCKS, SAND, FABRIC }
```
...
- **Non-null** means ML is confident this is one of the five canonical materials (WOOD / PAPER / ROCKS / SAND / FABRIC). The polished hand-tuned recipe path runs. Audio plays the curated sample loop; haptic fires the per-material signature on touch-down and the per-material drag stream during swipes.
...
| --- | --- | --- | --- |
| Wood | `THUD 0.70 + 80 ms + TICK 0.40` | `THUD` | 0.75 |
| Paper | `5x TICK 0.55 @ 40 ms` | `TICK` | 0.55 |
| Rocks | `8x LOW_TICK 0.80 @ 60 ms` | `LOW_TICK` | 0.75 |
| Sand | `16x LOW_TICK 0.35 @ 28 ms` | `LOW_TICK` | 0.60 |
| Fabric | `6x LOW_TICK 0.45 @ 50 ms + SLOW_RISE 0.40 @ 60 ms` | `LOW_TICK` | 0.45 |
...
| PAPER | ~46 px | Medium-dense, smooth with subtle grain |
| FABRIC | ~50 px | Fine weave, between paper and wood |
